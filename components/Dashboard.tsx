...
import {
   ...
   Search, Bell, ChevronDown, Briefcase, MoreVertical, X, MapPin, Filter, LayoutTemplate, Trash2, Check, RefreshCw, Activity, Link2, Plug, ExternalLink, BarChart2, DollarSign, CreditCard, Receipt, ArrowUp, ArrowDown, FileEdit, AlignLeft, AlertTriangle, Clock, Calendar, Twitter, Video, MousePointerClick, User, Lock, Palette, Download, ShoppingBag, Handshake,
   TrendingUp,
   Target
} from 'lucide-react';
...
// Search Console context and components
import { SearchConsoleProvider } from '../context/SearchConsoleContext';
import { ConnectButton } from './SearchConsole/ConnectButton';
import { KeywordTrends } from './SearchConsole/KeywordTrends';
import { KeywordTable } from './SearchConsole/KeywordTable';
import { ExportButton } from './SearchConsole/ExportButton';
import { KeywordTrendsChart } from './SearchConsole/KeywordTrendsChart';
import { GMBProfileProvider, useGMBProfile } from '../context/GMBProfileContext';
import { GMBConnectButton } from './GMBProfile/ConnectButton';
import { GMBTrends } from './GMBProfile/Trends';
import { GMBTable } from './GMBProfile/Table';
import { GMBExportButton } from './GMBProfile/ExportButton';
import { GMBTrendsChart } from './GMBProfile/TrendsChart';
...
import { Subscription, getCurrentSubscription, cancelSubscription, checkTrialExpiry } from '../services/subscriptionService';
...
const SidebarItem: React.FC<{ icon: React.ElementType, label: string, active?: boolean, onClick: () => void }> = ({ icon: Icon, label, active, onClick }) => (
   <div
      onClick={onClick}
      className={`flex items-center px-4 py-3 mb-1 cursor-pointer rounded-lg transition-colors ${active
...
// End Integrations/Platform Widgets Section
...
const Dashboard: React.FC = () => {
   ...
   // Search Console linking state
   const [gscLinked, setGscLinked] = useState<boolean>(false);
   ...
   // Google Ads linking state
   const [adsLinked, setAdsLinked] = useState<boolean>(false);
   const [adsCustomers, setAdsCustomers] = useState<string[]>([]);
   const [adsCustomerId, setAdsCustomerId] = useState<string>('');
   // Meta linking state
   const [metaLinked, setMetaLinked] = useState<boolean>(false);
   ...
   // Shopify linking state
   const [shopifyLinked, setShopifyLinked] = useState<boolean>(() => {
      const saved = localStorage.getItem('wpm_shopify_linked');
      return saved === 'true';
   });
   const [shopifyStoreId, setShopifyStoreId] = useState<string>(() => {
      return localStorage.getItem('wpm_shopify_store_id') || 'demo_store';
   });
   // HubSpot linking state
   const [hubspotLinked, setHubspotLinked] = useState<boolean>(() => {
      const saved = localStorage.getItem('wpm_hubspot_linked');
      return saved === 'true';
   });
   const [hubspotPortalId, setHubspotPortalId] = useState<string>(() => {
      return localStorage.getItem('wpm_hubspot_portal_id') || 'demo_portal';
   });
   // X linking state
   const [xLinked, setXLinked] = useState<boolean>(false);
   // LinkedIn linking state
   const [linkedInLinked, setLinkedInLinked] = useState<boolean>(false);
   const [linkedInOrgs, setLinkedInOrgs] = useState<Array<{ id: string; name: string }>>([]);
   const [linkedInOrgId, setLinkedInOrgId] = useState<string>('');
   // Filtering state
         ...
         if (integration.status === 'Connected') {
            try {
               if (integration.id === 'google_ads') {
                  const cust = adsCustomerId || '0000000000';
                  const adsData = await GoogleAdsService.getCampaignPerformance(cust, dateRange);
                  ...
                  const acc = metaAccountId || 'act_demo';
                  stats['meta_ads'] = await MetaAdsService.fetchData(acc, dateRange);
               } else if (integration.id === 'search_console') {
                  const site = gscSiteUrl || 'https://client-site.com';
                  stats['search_console'] = await SearchConsoleService.getSeoPerformance(site, dateRange);
               } else if (integration.id === 'linkedin') {
                  const org = linkedInOrgId || 'urn:li:organization:demo';
                  stats['linkedin'] = await LinkedInService.fetchData(org, dateRange);
               } else if (integration.id === 'x_ads') {
                  const xId = user?.id || 'demo';
                  stats['x_ads'] = await XService.fetchData(xId, dateRange);
                  ...
                  const storeId = shopifyStoreId || user?.id || 'demo_store';
                  stats['shopify'] = await ShopifyService.fetchData(storeId, dateRange);
               } else if (integration.id === 'hubspot_crm') {
                  const portalId = hubspotPortalId || user?.id || 'demo_portal';
                  stats['hubspot_crm'] = await HubSpotService.fetchData(portalId, dateRange);
   ...
   useEffect(() => {
      ...
      refreshData();
   }, [currentTenantId, dateRange]); // Re-fetch when date range or tenant changes

   useEffect(() => {
      fetchSubscriptionStatus();
   }, []);

   // Initial check for Search Console link and sites + Google Ads customers
   useEffect(() => {
      const initGsc = async () => {
         try {
            const linked = await SearchConsoleService.isLinked();
            setGscLinked(linked);
            if (linked) {
               const sites = await SearchConsoleService.listSites();
               setGscSites(sites);
               if (!gscSiteUrl && sites[0]?.siteUrl) setGscSiteUrl(sites[0].siteUrl);
            }
         } catch { }
      };
      const initAds = async () => {
         try {
            const linked = await GoogleAdsService.isLinked();
            setAdsLinked(linked);
            if (linked) {
               const customers = await GoogleAdsService.listCustomers();
               setAdsCustomers(customers);
               if (!adsCustomerId && customers[0]) setAdsCustomerId(customers[0]);
            }
         } catch { }
      };
      const initMeta = async () => {
         try {
            const linked = await MetaAdsService.isLinked();
            setMetaLinked(linked);
            if (linked) {
               const accounts = await MetaAdsService.listAccounts();
               setMetaAccounts(accounts);
               if (!metaAccountId && accounts[0]?.id) setMetaAccountId(accounts[0].id);
            }
         } catch { }
      };
      const initX = async () => {
         try {
            const linked = await XService.isLinked();
            setXLinked(linked);
         } catch { }
      };
      const initLinkedIn = async () => {
         try {
            const linked = await LinkedInService.isLinked();
            setLinkedInLinked(linked);
            if (linked) {
               const orgs = await LinkedInService.listOrganizations();
               setLinkedInOrgs(orgs);
               if (!linkedInOrgId && orgs[0]?.id) setLinkedInOrgId(orgs[0].id);
            }
         } catch { }
      };
      initGsc();
   ...
   const handleConnectSearchConsole = async () => {
      try {
         setConnectingId('search_console');
         const url = await SearchConsoleService.getOAuthUrl();
         const popup = window.open(url, 'gsc_oauth', 'width=480,height=700');
   ...
   const handleConnectGoogleAds = async () => {
      try {
         setConnectingId('google_ads');
         const url = await GoogleAdsService.getOAuthUrl();
         const popup = window.open(url, 'google_ads_oauth', 'width=480,height=700');
         const start = Date.now();
         const timeoutMs = 2 * 60 * 1000;
         const poll = async () => {
            const linked = await GoogleAdsService.isLinked();
   ...
   const handleConnectMeta = async () => {
      try {
         setConnectingId('meta_ads');
         const url = await MetaAdsService.getOAuthUrl();
         const popup = window.open(url, 'meta_oauth', 'width=480,height=700');
         const start = Date.now();
         const timeoutMs = 2 * 60 * 1000;
         const poll = async () => {
            const linked = await MetaAdsService.isLinked();
            if (linked) {
               ...
               if (popup && !popup.closed) popup.close();
               setConnectingId(null);
               addToast('Meta (Facebook/Instagram) connected!', 'success');
               refreshData();
               return;
            }
            if (Date.now() - start < timeoutMs) {
   ...
   const handleConnectShopify = async () => {
      if (!shopifyStoreId.trim()) {
         addToast('Enter a Shopify store ID to connect.', 'warning');
         return;
      }
      setConnectingId('shopify');
      setTimeout(() => {
         const updated = DataService.toggleIntegration('shopify');
         setIntegrations(updated);
         setShopifyLinked(true);
         localStorage.setItem('wpm_shopify_linked', 'true');
         localStorage.setItem('wpm_shopify_store_id', shopifyStoreId);
         setConnectingId(null);
         addToast('Shopify connected with custom store ID.', 'success');
         refreshData();
      }, 400);
   };

   const handleConnectHubSpot = async () => {
      if (!hubspotPortalId.trim()) {
         addToast('Enter a HubSpot portal ID to connect.', 'warning');
         return;
      }
      setConnectingId('hubspot_crm');
      setTimeout(() => {
         const updated = DataService.toggleIntegration('hubspot_crm');
         setIntegrations(updated);
         setHubspotLinked(true);
         localStorage.setItem('wpm_hubspot_linked', 'true');
         localStorage.setItem('wpm_hubspot_portal_id', hubspotPortalId);
         setConnectingId(null);
         addToast('HubSpot CRM connected with selected portal.', 'success');
         refreshData();
      }, 400);
   };
   ...
   const handleConnectX = async () => {
      try {
         setConnectingId('x_ads');
         const url = await XService.getOAuthUrl();
         const popup = window.open(url, 'x_oauth', 'width=480,height=700');
         const start = Date.now();
         const timeoutMs = 2 * 60 * 1000;
         const poll = async () => {
            const linked = await XService.isLinked();
            if (linked) {
               ...
               if (popup && !popup.closed) popup.close();
               setConnectingId(null);
               addToast('X (Twitter) connected!', 'success');
               refreshData();
               return;
            }
            if (Date.now() - start < timeoutMs) {
   ...
   const handleConnectLinkedIn = async () => {
      try {
         ...
         const url = await LinkedInService.getOAuthUrl();
         const popup = window.open(url, 'linkedin_oauth', 'width=480,height=700');
         const start = Date.now();
         const timeoutMs = 2 * 60 * 1000;
         const poll = async () => {
            const linked = await LinkedInService.isLinked();
            if (linked) {
               ...
               if (popup && !popup.closed) popup.close();
               setConnectingId(null);
               addToast('LinkedIn connected!', 'success');
               refreshData();
               return;
            }
            if (Date.now() - start < timeoutMs) {
   ...
   const computeDaysFromDate = (dateStr?: string | null) => {
      if (!dateStr) return 0;
   ...
   const subscriptionStatus = subscription?.status || (user?.isTrial ? 'trial' : 'none');
   const trialCountdown = subscriptionStatus === 'trial' ? (trialDaysRemaining || getDaysRemaining()) : 0;
   const nextBillingDays = subscription?.nextBillingDate ? computeDaysFromDate(subscription.nextBillingDate) : 0;
   const subscriptionPlanLabel = subscription?.planId ? subscription.planId.charAt(0).toUpperCase() + subscription.planId.slice(1) : 'No Plan';
   ...
   const fetchSubscriptionStatus = async () => {
      try {
         ...
         if (result.subscription?.status === 'trial') {
            const daysLeft = result.subscription.trialDaysRemaining ?? computeDaysFromDate(result.subscription.trialEndsAt);
            setTrialDaysRemaining(Math.max(0, daysLeft));
         }

         // Check if trial expired and surface UI
         const trialStatus = await checkTrialExpiry();
         if (trialStatus.expired) {
            setTrialExpired(true);
            setShowPackageSelection(true);
         } else if (trialStatus.trialActive && typeof trialStatus.daysRemaining === 'number') {
            setTrialDaysRemaining(Math.max(0, trialStatus.daysRemaining));
         }
      } catch (error) {
   ...
   const handleAddClient = (e: React.FormEvent) => {
      e.preventDefault();
      if (!canManageClients) {
         addToast('You do not have permission to manage clients', 'error');
         return;
      }
   ...
   const handleCreateTemplate = (e: React.FormEvent) => {
      e.preventDefault();
      if (!canCreateReports) {
         addToast('You do not have permission to create templates', 'error');
         return;
      }
   ...
   const handleDeleteTemplate = (id: string) => {
      if (!canDeleteReports) {
         addToast('You do not have permission to delete templates', 'error');
         return;
      }
   ...
   const confirmGenerateReport = () => {
      ...
      if (!canCreateReports) {
         addToast('You do not have permission to create reports', 'error');
   ...
   const handleToggleIntegration = async (id: string) => {
      if (!canManageIntegrations) {
         addToast('You do not have permission to manage integrations', 'error');
         return;
      }
      setConnectingId(id);
      ...
      // Clear Shopify/HubSpot state on disconnect
      if (!wasConnected) {
         if (id === 'shopify') {
            setShopifyLinked(false);
            localStorage.removeItem('wpm_shopify_linked');
         } else if (id === 'hubspot_crm') {
            setHubspotLinked(false);
            localStorage.removeItem('wpm_hubspot_linked');
         }
      }
   ...
   const handleCreateInvoice = (e: React.FormEvent) => {
      e.preventDefault();
      if (!canManageBilling) {
         addToast('You do not have permission to manage billing', 'error');
         return;
      }
      const formData = new FormData(e.target as HTMLFormElement);
   ...
   const handlePaymentSuccess = async () => {
      ...
      try {
         ...
         addToast('Payment successful, but failed to refresh invoice list', 'warning');
      }
   };

   const handleExportReport = async (reportId: string, format: 'pdf' | 'csv' | 'excel' = 'pdf') => {
      if (!canExportData) {
      ...
      try {
         ...
         const response = await fetch(`${config.API_BASE_URL}/reports/${reportId}/export/${format}`, {
            headers: {
               'Authorization': `Bearer ${localStorage.getItem('wpm_auth_token')}`
            }
         });
      ...
      <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
         ...
         <aside className="w-64 bg-brand-900 text-white flex flex-col border-r border-brand-800 hidden md:flex">
            ...
            <div className="flex-1 px-3 py-6 overflow-y-auto">
               ...
               <SidebarItem icon={MapPin} label="GMB Profile" active={activeTab === 'gmb_profile'} onClick={() => setActiveTab('gmb_profile')} />
               <SidebarItem icon={Search} label="Keyword Rankings" active={activeTab === 'keyword_rankings'} onClick={() => setActiveTab('keyword_rankings')} />
         ...
         <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
            ...
            <header className="bg-white shadow-sm z-10 h-16 flex items-center justify-between px-6 lg:px-8">
               ...
               <div className="flex items-center space-x-4">
                     ...
                     <div className="hidden md:flex items-center bg-gray-50 rounded-lg p-1 border border-gray-200">
                        {(['daily', 'weekly', 'monthly'] as const).map(range => (
                           <button
                              ...
                              className={`px-3 py-1.5 text-xs font-medium rounded-md transition-all capitalize ${dateRange === range
                  ...
                  <Button
                     ...
                     title={!canManageClients ? 'Requires manage clients permission' : undefined}
                  >
            ...
            <main className="flex-1 overflow-y-auto p-6 lg:p-8">
               ...
               {/* Show Package Selection if trial expired */}
               ...
               {!showPackageSelection && (
                  ...
                  ) : activeTab === 'keyword_rankings' ? (
                     <SearchConsoleProvider>
                        <div className="space-y-6">
                           <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                              <Search size={22} className="text-green-600" /> Keyword Rankings (Google Search Console)
                           </h2>
                           <div className="mb-6">
                              <ConnectButton />
                           </div>
                  ...
                  ) : activeTab === 'gmb_profile' ? (
                     <GMBProfileProvider>
                        <div className="space-y-6">
                           <h2 className="text-2xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                              <MapPin size={22} className="text-blue-600" /> Google My Business Profile
                           </h2>
                           <div className="mb-6">
                              <GMBConnectButton />
                           </div>
                           <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                 <GMBTrends data={[]} />
                                 <GMBTable rows={[]} />
                                 <GMBExportButton onExport={() => { }} />
                              </div>
                              <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                                 <GMBTrendsChart chartData={[]} />
                              </div>
                           </div>
                        </div>
                     </GMBProfileProvider>
...
// ...existing code...
               ...
               {/* Google Ads Widget + Connect/Select controls */}
               <div className="space-y-3">
                  {!adsLinked && (
                     <div className="bg-blue-50 border border-blue-200 text-blue-800 rounded-lg p-3 flex items-center justify-between">
                        <div className="text-sm">Connect Google Ads to see live campaign KPIs</div>
                        <Button size="sm" variant="primary" onClick={handleConnectGoogleAds} disabled={connectingId === 'google_ads'}>
                           {connectingId === 'google_ads' ? <span className="flex items-center"><RefreshCw size={14} className="animate-spin mr-1" />Connecting</span> : 'Connect Google Ads'}
                        </Button>
                     </div>
                  )}
                  {adsLinked && (
                     <div className="flex items-center gap-2">
                        ...
                        <select
                           ...
                           aria-label="Google Ads customer"
                        >
                           {adsCustomers.map(id => (
                              <option key={id} value={id}>{id}</option>
                           ))}
                        </select>
                        <Button size="sm" variant="outline" onClick={refreshData}><RefreshCw size={14} className="mr-1" />Refresh</Button>
                     </div>
                  )}
                  <PlatformWidget
               ...
               {/* Meta Ads Widget + Connect/Select controls */}
               <div className="space-y-3">
                  {!metaLinked && (
                     <div className="bg-purple-50 border border-purple-200 text-purple-800 rounded-lg p-3 flex items-center justify-between">
                        <div className="text-sm">Connect Meta to see Facebook & Instagram Ads data</div>
                        <Button size="sm" variant="primary" onClick={handleConnectMeta} disabled={connectingId === 'meta_ads'}>
                           {connectingId === 'meta_ads' ? <span className="flex items-center"><RefreshCw size={14} className="animate-spin mr-1" />Connecting</span> : 'Connect Meta'}
                        </Button>
                     </div>
                  )}
                  {metaLinked && metaAccounts.length > 0 && (
                     <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-600">Ad Account:</span>
                        <select
                           className="text-sm border border-gray-200 rounded-md px-2 py-1 bg-white"
                           value={metaAccountId}
               ...
               {/* X (Twitter) Widget + Connect controls */}
               <div className="space-y-3">
                  {!xLinked && (
                     <div className="bg-sky-50 border border-sky-200 text-sky-800 rounded-lg p-3 flex items-center justify-between">
                        <div className="text-sm">Connect X to see your Twitter metrics</div>
                        <Button size="sm" variant="primary" onClick={handleConnectX} disabled={connectingId === 'x_ads'}>
                           {connectingId === 'x_ads' ? <span className="flex items-center"><RefreshCw size={14} className="animate-spin mr-1" />Connecting</span> : 'Connect X'}
                        </Button>
                     </div>
                  )}
                  <PlatformWidget
                     ...
                     error={apiErrors.x_ads}
                  />
               </div>

               {/* LinkedIn Widget + Connect/Select controls */}
               <div className="space-y-3">
                  {!linkedInLinked && (
                     <div className="bg-blue-50 border border-blue-200 text-blue-700 rounded-lg p-3 flex items-center justify-between">
                        <div className="text-sm">Connect LinkedIn to see your professional network data</div>
                        <Button size="sm" variant="primary" onClick={handleConnectLinkedIn} disabled={connectingId === 'linkedin'}>
                           {connectingId === 'linkedin' ? <span className="flex items-center"><RefreshCw size={14} className="animate-spin mr-1" />Connecting</span> : 'Connect LinkedIn'}
                        </Button>
                     </div>
                  )}
                  {linkedInLinked && linkedInOrgs.length > 0 && (
                     <div className="flex items-center gap-2">
                  ...
                  <PlatformWidget
                     ...
                     error={apiErrors.linkedin}
                  />
               </div>

               {/* Search Console Widget + Connect/Select controls */}
               <div className="space-y-3">
                  {!gscLinked && (
                     <div className="bg-green-50 border border-green-200 text-green-800 rounded-lg p-3 flex items-center justify-between">
                        <div className="text-sm">Connect Google Search Console to see live SEO data</div>
                        <Button size="sm" variant="primary" onClick={handleConnectSearchConsole} disabled={connectingId === 'search_console'}>
                           {connectingId === 'search_console' ? <span className="flex items-center"><RefreshCw size={14} className="animate-spin mr-1" />Connecting</span> : 'Connect Search Console'}
                        </Button>
                     </div>
                  )}
                  {gscLinked && (
...
// ...existing code...
                  ...
                  <PlatformWidget
                     ...
                     error={apiErrors.tiktok_ads}
                  />

                  {/* Shopify Widget + Connect controls */}
                  <div className="space-y-3">
                     {!shopifyLinked && (
                        <div className="bg-emerald-50 border border-emerald-200 text-emerald-800 rounded-lg p-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                           <div className="text-sm">Connect Shopify to pull revenue, orders, and repeat rate. Enter your store ID.</div>
                           <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                              <input
                                 className="text-sm border border-emerald-200 rounded-md px-2 py-1 bg-white"
                                 placeholder="example-store"
                                 value={shopifyStoreId}
                                 onChange={(e) => setShopifyStoreId(e.target.value)}
                                 aria-label="Shopify store ID"
                              />
                              <Button size="sm" variant="primary" onClick={handleConnectShopify} disabled={connectingId === 'shopify'}>
                                 {connectingId === 'shopify' ? <span className="flex items-center"><RefreshCw size={14} className="animate-spin mr-1" />Connecting</span> : 'Connect Shopify'}
                              </Button>
                           </div>
                        </div>
                     )}
                     {shopifyLinked && (
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                           <span className="text-xs text-gray-600">Store ID:</span>
                           <input
                              className="text-sm border border-gray-200 rounded-md px-2 py-1 bg-white"
                              value={shopifyStoreId}
                              onChange={(e) => setShopifyStoreId(e.target.value)}
                              aria-label="Shopify store id connected"
                           />
                           <Button size="sm" variant="outline" onClick={refreshData}><RefreshCw size={14} className="mr-1" />Refresh</Button>
                        </div>
                     )}
                     <PlatformWidget
                        data={platformStats['shopify'] || null}
                        title="Shopify Commerce"
                        icon={ShoppingBag}
                        colorClass="text-emerald-600"
                        error={apiErrors.shopify}
                     />
                  </div>

                  {/* HubSpot CRM Widget + Connect controls */}
                  <div className="space-y-3">
                     {!hubspotLinked && (
                        <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg p-3 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                           <div className="text-sm">Connect HubSpot to sync deals and pipeline. Enter your portal ID.</div>
                           <div className="flex flex-col sm:flex-row gap-2 sm:items-center">
                              <input
                                 className="text-sm border border-amber-200 rounded-md px-2 py-1 bg-white"
                                 placeholder="123456"
                                 value={hubspotPortalId}
                                 onChange={(e) => setHubspotPortalId(e.target.value)}
                                 aria-label="HubSpot portal ID"
                              />
                              <Button size="sm" variant="primary" onClick={handleConnectHubSpot} disabled={connectingId === 'hubspot_crm'}>
                                 {connectingId === 'hubspot_crm' ? <span className="flex items-center"><RefreshCw size={14} className="animate-spin mr-1" />Connecting</span> : 'Connect HubSpot'}
                              </Button>
                           </div>
                        </div>
                     )}
                     {hubspotLinked && (
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2">
                           ...
                           <input
                              ...
                              value={hubspotPortalId}
                              onChange={(e) => setHubspotPortalId(e.target.value)}
                              aria-label="HubSpot portal id connected"
                           />
                           <Button size="sm" variant="outline" onClick={refreshData}><RefreshCw size={14} className="mr-1" />Refresh</Button>
                        </div>
                     )}
                     <PlatformWidget
                        data={platformStats['hubspot_crm'] || null}
                        title="HubSpot CRM"
                        icon={Handshake}
                        colorClass="text-amber-600"
                        error={apiErrors.hubspot_crm}
                     />
                  </div>
                  )
                  {/* End Integrations/Platform Widgets Section */}
                              ...
                              {clients.map((client) => (
                                 <div key={client.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all group">
                                    ...
                                    <div className="mt-4 pt-4 border-t border-gray-50 flex items-center justify-between">
                                       <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${client.status === 'Active' ? 'bg-green-100 text-green-700' :
                                    ...
                                    <div key={report.id} className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
                                       <div className="flex justify-between items-start mb-4">
                                          ...
                                          <div className="flex flex-col items-end">
                                             <span className={`px-2 py-0.5 text-[10px] uppercase tracking-wide font-bold rounded mb-1 ${report.status === 'Sent' ? 'bg-green-100 text-green-700' :
                                       ...
                                       <div className="flex items-center justify-between pt-4 border-t border-gray-50 text-xs text-gray-500">
                                          ...
                                          <div className="flex gap-2">
                                             <button onClick={() => handleExportReport(report.id, 'pdf')} className="text-brand-600 font-medium hover:underline flex items-center gap-1">
                                                <Download size={12} /> PDF
                                             </button>
                                             <button onClick={() => handleExportReport(report.id, 'csv')} className="text-gray-500 font-medium hover:underline flex items-center gap-1">
                                                CSV
                                             </button>
                                          </div>
                                       </div>
                           ...
                           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                              {templates.map((template) => (
                                 <div key={template.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-all flex flex-col relative group">
                                    <div className="flex justify-between items-start mb-4">
                                       <span className={`px-2.5 py-0.5 rounded-md text-xs font-bold uppercase ${template.isCustom ? 'bg-purple-100 text-purple-700' : 'bg-brand-100 text-brand-700'
                                          }`}>
                           ...
                           {integrations.map((integration) => {
                              ...
                              return (
                                 <div key={integration.id} className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm flex flex-col">
                                    <div className="flex justify-between items-start mb-4">
                                       <div className={`w-12 h-12 rounded-lg flex items-center justify-center text-2xl ${integration.provider === 'google' ? 'bg-blue-50 text-blue-600' :
                                             ...
                                             integration.provider === 'linkedin' ? 'bg-blue-50 text-blue-800' :
                                                integration.provider === 'tiktok' ? 'bg-pink-50 text-pink-500' :
                                                   integration.provider === 'x' ? 'bg-gray-100 text-gray-900' :
                                                      integration.provider === 'shopify' ? 'bg-emerald-50 text-emerald-600' :
                                                         integration.provider === 'hubspot' ? 'bg-amber-50 text-amber-600' :
                                                   ...
                                                   integration.id === 'linkedin' ? <Briefcase size={24} /> :
                                                      integration.id === 'x_ads' ? <Twitter size={24} /> :
                                                         integration.id === 'tiktok_ads' ? <Video size={24} /> :
                                                            integration.id === 'shopify' ? <ShoppingBag size={24} /> :
                                                               integration.id === 'hubspot_crm' ? <Handshake size={24} /> :
                                                                  <Link2 size={24} />
                                          }
                                       </div>
                                       <span className={`px-2 py-1 rounded text-xs font-bold uppercase ${statusLabel === 'Connected' ? 'bg-green-100 text-green-700' :
                                       ...
                                       {integration.status === 'Connected' && !hasError ? (
                                          ...
                                          <Button
                                             ...
                                             onClick={() => {
                                                ...
                                                if (integration.id === 'shopify') return handleConnectShopify();
                                                if (integration.id === 'hubspot_crm') return handleConnectHubSpot();
                     ...
                     <div className="max-w-4xl mx-auto space-y-8">
                        ...
                        {user?.role === 'ADMIN' && (
                           <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
                              <div className="p-6 border-b border-gray-100">
                                 <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                                    <CreditCard size={20} className="text-green-600" /> Payment Settings
                                 </h3>
                                 <p className="text-sm text-gray-500 mt-1">Configure payment gateways and bank account details.</p>
                              </div>
                              <div className="p-6">
                                 <PaymentSettings />
                              </div>
            ...
            {/* End of main dashboard content wrapper */}
            ...
            {/* Old Payment Modal - Remove this after testing */}
            {false && isPaymentModalOpen && selectedInvoice && (
               <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
                  <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 m-4">
                     <div className="flex justify-between items-center mb-6">
                                                   ...
                                                   <div className="flex bg-gray-100 rounded-md p-1">
                                                      {(['bar', 'line', 'area'] as const).map((type) => (
                                                         <button
                                                            ...
                                                            className={`px-3 py-1 text-xs font-medium rounded-sm capitalize transition-all ${widget.chartType === type ? 'bg-white shadow text-brand-600' : 'text-gray-500 hover:text-gray-700'
                                                               }`}
                                                         >
                                                            {type}
                                                         </button>
